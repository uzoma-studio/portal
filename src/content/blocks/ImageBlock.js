...
const ImageBlock = ({ id, image, position, size, linkToPage, index, isBuildMode, isCurrentEditImage }) => {
  return (
    <Rnd
        default={{
            x: position.x,
        ...
        className={isBuildMode && isCurrentEditImage ? 'active-edit-image' : ''}
    >
        <StyledSpaceImage
            $zIndex={index+1}
            $hasLink={!!linkToPage}
            $isCurrentEditImage={isCurrentEditImage}
...
export const StyledSpaceImage = styled.div`
    position: absolute;
    object-fit: contain;
    cursor: ${props => props.$hasLink ? 'pointer' : (props.$isCurrentEditImage ? 'move' : 'default')};
